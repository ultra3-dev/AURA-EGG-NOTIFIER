...
local base = "https://raw.githubusercontent.com/ultra3-dev/AURA-EGG-NOTIFIER/restaurar-0225220/"
local files = {
	"AURA_EGG_BOOT.lua",
...
if fileName == "AURA_EGG_DETECTION.lua" then
local wrapper = "local function runNotifierRuntime()\n"
local wrapperStart = source:find(wrapper, 1, true)
if wrapperStart then
source = source:sub(1, wrapperStart - 1)
.. source:sub(wrapperStart + #wrapper)
end
elseif fileName == "AURA_EGG_DELIVERY.lua" then
local runtimeCall = "\nrunNotifierRuntime()"
local runtimeCallStart = source:find(runtimeCall, 1, true)
if runtimeCallStart then
source = source:sub(1, runtimeCallStart - 1)
.. source:sub(runtimeCallStart + #runtimeCall)
end
end
